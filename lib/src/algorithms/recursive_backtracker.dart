...
class RecursiveBacktracker extends Base {
  ...
  RecursiveBacktracker(Maze maze, MazeOptions options) : super(maze, options) {
    //#:nodoc:
    while (true) {
    ...
    _tries = List.from(_maze!.potential_exits_at(_x, _y))
      ..shuffle(ruby.getRandom());
    _stack = <List<dynamic>>[];
  }
  ...
  bool do_step() {
    ...
    _tries = List.from(_maze!.potential_exits_at(nx, ny))
      ..shuffle(ruby.getRandom());
    //TODO: Check syntax
    if (!(ruby.rand(100) < _maze!.randomness!) && _tries!.contains(direction)) {
      //_tries.include?(direction) unless rand(100) < _maze.randomness
  ...
  int? _next_direction() {
    ...
    while (true) {
      ...
      if (_maze!.valid(nx, ny) &&
          (_maze!.getCell(_x, _y) &
                  (direction! | (direction << Maze.UNDER_SHIFT)) ==
              0)) {
        if (_maze!.getCell(nx, ny) == 0) {
          return direction;
        } else if (!_maze!.dead(_maze!.getCell(nx, ny)) &&
            _maze!.weave! > 0 &&
            ruby.rand(100) < _maze!.weave!) {
