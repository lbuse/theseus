...
class DeltaMaze extends Maze {
  DeltaMaze(MazeOptions options) : super(options) {
    //#:nodoc:
    if (_weave! > 0) {
      throw ArgumentError("weaving is not supported for delta mazes");
    }
  }
  ...
  V to<V, P>(FormatType format, [P? options]) {
    if (format == FormatType.ascii) {
      return formatters.ASCIIDelta(this) as V;
    } else if (format == FormatType.png) {
      return formatters.PNGDelta(
