...
class ASCIIOrthogonal extends ASCII {
  ...
  static Dimensions dimensions_for(Maze maze, [ASCIIMode? mode]) {
    ...
    switch (mode) {
      case ASCIIMode.plain:
        return Dimensions(maze.width! * 2 + 1, maze.height! + 1);
      case ASCIIMode.unicode:
        return Dimensions(maze.width! * 3, maze.height! * 2);
      case ASCIIMode.lines:
        return Dimensions(maze.width, maze.height);
      default:
  ...
  //# for this class.
  ASCIIOrthogonal(OrthogonalMaze maze, [ASCIIMode mode = ASCIIMode.plain])
      : super(dimensions_for(maze, mode).width,
            dimensions_for(maze, mode).height!) {
    if (mode == null) {
      mode = ASCIIMode.plain;
    }
...
//          super(width, height)
    ...
    for (int y = 0; y < maze.height!; y++) {
      ...
      for (int x = 0; x < length; x++) {
        switch (mode) {
          case ASCIIMode.plain:
            _draw_plain_cell(maze, x, y);
            break;
          case ASCIIMode.unicode:
            _draw_unicode_cell(maze, x, y);
            break;
          case ASCIIMode.lines:
            _draw_line_cell(maze, x, y);
            break;
        }
      }
    }
  ...
  void _draw_plain_cell(Maze maze, int x, int y) {
    //#:nodoc:
    ...
    if (c & Maze.N == 0) {
      if (y == 0 ||
          (cn == 0 && cnw == 0) ||
          cnw & (Maze.E | Maze.S) == Maze.E) {
        setCell(px, py, "_");
      }
      setCell(px + 1, py, "_");
      if (y == 0 ||
          (cn == 0 && cne == 0) ||
          cne & (Maze.W | Maze.S) == Maze.W) {
        setCell(px + 2, py, "_");
      }
    }
    ...
    if (c & Maze.S == 0) {
      ...
      if (bottom ||
          (cs == 0 && csw == 0) ||
          csw & (Maze.E | Maze.N) == Maze.E) {
        setCell(px, py + 1, "_");
      }
      setCell(px + 1, py + 1, "_");
      if (bottom ||
          (cs == 0 && cse == 0) ||
          cse & (Maze.W | Maze.N) == Maze.W) {
        setCell(px + 2, py + 1, "_");
      }
    }
  ...
  void _draw_unicode_cell(Maze maze, int x, int y) {
    //#:nodoc:
    var cx = 3 * x, cy = 2 * y;
    var cell = maze.getCell(x, y);

    ruby.each_with_index<String>(
      UTF8_SPRITES[cell & Maze.PRIMARY],
      (row, sy) {
  ...
  void _draw_line_cell(Maze maze, int x, int y) {
    //#:nodoc:
