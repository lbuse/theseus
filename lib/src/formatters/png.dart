...
//        });
    ...
    if (_options!.solution != null) {
      //TODO: Implement!
      throw UnimplementedError("options.solution");
...
//          _paths = [path, *_paths];
  ...
  int? color_at(Position pt, [int? direction = null]) {
    ...
    for (Path path in _paths) {
      if (direction != null ? path.path(pt, direction) : path.isSet(pt)) {
        return path.color;
      }
    }
  ...
  Position move(Position point, num dx, num dy) {
    return Position.xy(point.x! + dx, point.y! + dy);
  }
  ...
  void _fill_rect(
      PNGCanvas canvas, num? x0, num? y0, num? x1, num? y1, int? color) {
  ...
  void _fill_poly(PNGCanvas canvas, List<Position> points, int? color) {
    ...
    for (int y = min_y!.floor(); y < max_y!.ceil(); y++) {
      ...
      points.forEach((Position point) {
        if (point.y! < y && prev!.y! >= y || prev!.y! < y && point.y! >= y) {
          nodes.add((point.x! +
              (y - point.y!) /*.to_f*/ /
                  (prev!.y! - point.y!) *
                  (prev!.x! - point.x!))); // <<
