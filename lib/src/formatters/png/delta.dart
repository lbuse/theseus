...
class PNGDelta extends PNG {
  ...
  PNGDelta(DeltaMaze maze, PNGFormatterOptions options) : super(maze, options) {
    ...
    var width =
        options.outer_padding * 2 + (maze.width! + 1) * options.cell_size / 2;
    ...
    for (int y = 0; y < maze.height!; y++) {
      ...
      for (int x = 0; x < maze.row_length(y); x++) {
        num px = options.outer_padding + x * options.cell_size / 2.0;
        _draw_cell(canvas, Position.xy(x, y), maze.points_up(x, y), px, py,
            maze.getCell(x, y));
      }
    }
  }
  ...
  void _draw_cell(
      PNGCanvas? canvas, Position point, bool up, num x, num y, int cell) {
    //#:nodoc:
    if (cell == 0) {
      return;
    }
    var p1 = Position<num>.xy(
        x + options!.cell_size / 2.0,
        up
            ? (y + options!.cell_padding)
            : (y + options!.cell_size - options!.cell_padding));
    var p2 = Position.xy(
        x + options!.cell_padding,
        up
            ? (y + options!.cell_size - options!.cell_padding)
            : (y + options!.cell_padding));
    var p3 = Position.xy(x + options!.cell_size - options!.cell_padding, p2.y);
