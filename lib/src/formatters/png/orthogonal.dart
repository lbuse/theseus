...
class PNGOrthogonal extends PNG {
  ...
  PNGOrthogonal(OrthogonalMaze maze, PNGFormatterOptions options)
    ...
    for (int y = 0; y < maze.height!; y++) {
      ...
      for (int x = 0; x < maze.width!; x++) {
        ...
        _draw_cell(canvas, Position.xy(x, y), px, py, maze.getCell(x, y));
      }
    }
  }
  ...
  void _draw_cell(PNGCanvas? canvas, Position point, int x, int y, int cell) {
    ...
    _draw_vertical(canvas, x, y, 1, north || north_under, !north || north_under,
        color_at(point, ANY_N));
    _draw_vertical(canvas, x, y + options!.cell_size, -1, south || south_under,
        !south || south_under, color_at(point, ANY_S));
    _draw_horizontal(canvas, x, y, 1, west || west_under, !west || west_under,
        color_at(point, ANY_W));
    _draw_horizontal(canvas, x + options!.cell_size, y, -1, east || east_under,
        !east || east_under, color_at(point, ANY_E));
  }

  void _draw_vertical(PNGCanvas? canvas, int x, int y, int direction,
    ...
    if (wall) {
      _fill_rect(canvas!, x + d1 - w1, y + (d1 - w1) * direction, x + d2 + w2,
          y + (d1 + w2) * direction, options!.wall_color);
    }
  }
