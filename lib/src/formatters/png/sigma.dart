  ...
  PNGSigma(SigmaMaze maze, PNGFormatterOptions options) : super(maze, options) {
    ...
    for (int y = 0; y < maze.height!; y++) {
      ...
      for (int x = 0; x < maze.row_length(y); x++) {
        ...
        _draw_cell(canvas, Position.xy(x, y), shifted, px, py + dy,
            maze.getCell(x, y));
      }
    }
  }
...
// any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
  ...
  void _draw_cell(
      PNGCanvas? canvas, Position point, bool shifted, num x, num y, int cell) {
    if (cell == 0) {
      return;
    }
    ...
    var p1 =
        Position.xy(x + options!.cell_padding + s4, y + options!.cell_padding);
    var p2 =
        Position.xy(x + options!.cell_size - options!.cell_padding - s4, p1.y);
    var p3 = Position.xy(
        x + options!.cell_padding + size, y + options!.cell_size / 2.0);
    var p4 = Position.xy(p2.x, y + options!.cell_size - options!.cell_padding);
    var p5 = Position.xy(p1.x, p4.y);
    var p6 = Position.xy(x + options!.cell_padding, p3.y);
...
//          any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
    ...
    if (cell & any(s) != 0) {
      ...
      _fill_rect(canvas, r1.x, r1.y, r2.x, r2.y, color_at(point, any(s)));
    ...
    if (cell & any(ne) != 0) {
      ...
      var ne_p5 = Position.xy(ne_x + options!.cell_padding + s4,
          ne_y + options!.cell_size - options!.cell_padding);
      var ne_p6 = Position.xy(
          ne_x + options!.cell_padding, ne_y + options!.cell_size * 0.5);
    ...
    if (cell & any(se) != 0) {
      ...
      var se_p1 = Position.xy(
          se_x + s4 + options!.cell_padding, se_y + options!.cell_padding);
      var se_p6 = Position.xy(
          se_x + options!.cell_padding, se_y + options!.cell_size * 0.5);
