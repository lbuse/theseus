...
class PNGUpsilon extends PNG {
  ...
  PNGUpsilon(UpsilonMaze maze, PNGFormatterOptions options)
    ...
    Metrics metrics =
        Metrics(options, size: options.cell_size - options.cell_padding * 2);
    ...
    for (int y = 0; y < maze.height!; y++) {
      ...
      for (int x = 0; x < maze.row_length(y); x++) {
        ...
        if ((y + x) % 2 == 0) {
          _draw_octogon_cell(canvas!, Position.xy(x, y), px, py, cell, metrics);
        } else {
          _draw_square_cell(canvas!, Position.xy(x, y), px, py, cell, metrics);
        }
      }
    }
  }
...
//        any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
  ...
  void _draw_octogon_cell(PNGCanvas canvas, Position point, num x, num y,
    ...
    var p3 = Position.xy(x + options!.cell_size - options!.cell_padding,
        y + options!.cell_padding + metrics.s4);
    var p4 = Position.xy(
        p3.x, y + options!.cell_size - options!.cell_padding - metrics.s4);
    var p5 = Position.xy(p2.x, y + options!.cell_size - options!.cell_padding);
    var p6 = Position.xy(p1.x, p5.y);
    var p7 = Position.xy(x + options!.cell_padding, p4.y);
    var p8 = Position.xy(p7.x, p3.y);
    ...
    if (cell & any(Maze.NE) != 0) {
      ...
      _fill_poly(
          canvas, [p2, far_p7, far_p6, p3], color_at(point, any(Maze.NE)));
    ...
    if (cell & any(Maze.E) != 0) {
      var edge =
          (x + options!.cell_size + options!.cell_padding > canvas.width);
      var r1 = p3;
      var r2 = edge
          ? move(p4, options!.cell_padding, 0)
          : move(p7, options!.cell_size, 0);
      _fill_rect(canvas, r1.x, r1.y, r2.x, r2.y, color_at(point, any(Maze.E)));
      _line(canvas, r1, Position.xy(r2.x, r1.y), options!.wall_color);
      _line(canvas, r2, Position.xy(r1.x, r2.y), options!.wall_color);
    }
    ...
    if (cell & any(Maze.SE) != 0) {
      ...
      _fill_poly(
          canvas, [p4, far_p1, far_p8, p5], color_at(point, any(Maze.SE)));
    ...
    if (cell & any(Maze.S) != 0) {
      ...
      _fill_rect(canvas, r1.x, r1.y, r2.x, r2.y, color_at(point, any(Maze.S)));
      _line(canvas, r1, Position.xy(r1.x, r2.y), options!.wall_color);
      _line(canvas, r2, Position.xy(r2.x, r1.y), options!.wall_color);
    }
  ...
  void _draw_square_cell(PNGCanvas canvas, Position point, num x, num y,
    ...
    var p1 = Position.xy(x + v, y + v);
    var p2 =
        Position.xy(x + options!.cell_size - v, y + options!.cell_size - v);
...
//          any = proc { |x| x | (x << Maze.UNDER_SHIFT) };
    ...
    if (cell & any(Maze.E) != 0) {
      var r1 = Position.xy(p2.x, p1.y);
      var r2 = Position.xy(x + metrics.inc + v, p2.y);
      _fill_rect(canvas, r1.x, r1.y, r2.x, r2.y, color_at(point, any(Maze.E)));
      _line(canvas, r1, Position.xy(r2.x, r1.y), options!.wall_color);
      _line(canvas, Position.xy(r1.x, r2.y), r2, options!.wall_color);
    }

    if (cell & any(Maze.S) != 0) {
      var r1 = Position.xy(p1.x, p2.y);
      var r2 = Position.xy(p2.x, y + metrics.inc + v);
      _fill_rect(canvas, r1.x, r1.y, r2.x, r2.y, color_at(point, any(Maze.S)));
      _line(canvas, r1, Position.xy(r1.x, r2.y), options!.wall_color);
      _line(canvas, Position.xy(r2.x, r1.y), r2, options!.wall_color);
    }

    if (cell & Maze.N == 0) {
      _line(canvas, p1, Position.xy(p2.x, p1.y), options!.wall_color);
    }
    if (cell & Maze.E == 0) {
      _line(canvas, Position.xy(p2.x, p1.y), p2, options!.wall_color);
    }
    if (cell & Maze.S == 0) {
      _line(canvas, Position.xy(p1.x, p2.y), p2, options!.wall_color);
    }
    if (cell & Maze.W == 0) {
      _line(canvas, p1, Position.xy(p1.x, p2.y), options!.wall_color);
    }
  }
}
