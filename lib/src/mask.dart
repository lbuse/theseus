...
//      _grid =  List.generator(image.height,(y){
//          return  List.generator( image.width,(x){return (image[x, y] & 0xff) == 0;});
//        });
//      return  Mask(grid);
    throw UnimplementedError();
  }
...
class TriangleMask extends BaseMask {
  ...
  TriangleMask(int height) {
    _height = height;
    _width = _height! * 2 + 1;
    _grid = List<List<bool>>.generate(_height!, (y) {
      var run = y * 2 + 1;
      var from = _height! - y;
      var to = from + run - 1;
      return List.generate(_width!, (x) {
