...
enum SymmetryType { x, y, xy, radial }

enum WrapType { x, y, xy }

enum FormatType { ascii, png }
...
//     throw  UnimplementedError("Maze.generate");
...
//   }
  ...
  Maze(MazeOptions options/*={}*/) {
    ...
    _symmetry =
        options.symmetry != null ? options.symmetry : /*none*/ null; //).to_sym
    _configure_symmetry();

    _randomness = options.randomness != null ? options.randomness : 100;
    _mask = options.mask != null ? options.mask : TransparentMask();
    ...
    _cells = _setup_grid();
    if (_cells == null) {
      throw Exception("expected #setup_grid to return the new grid");
    }

    _entrance =
        options.entrance != null ? options.entrance : _default_entrance();
    _exit = options.exit != null ? options.exit : _default_exit();

    var algorithm_class =
        options.algorithm != null ? options.algorithm : RecursiveBacktracker;
...
//         return new klass(self, a, b);
    throw UnimplementedError("new_solver");
    //TODO: fix
  }
  ...
  List<int> potential_exits_at(int x, int? y) {
    throw UnimplementedError("subclasses must implement //#potential_exits_at");
  }
  ...
  List<Position> dead_ends() {
    ...
    ruby.each_with_index<List<int>>(
      _cells!,
      /*_cells.each_with_index do |row, y|*/
      (row, y) {
        ruby.each_with_index<int>(
          row,
          (cell, x) {
            /*row.each_with_index do |cell, x|*/
            if (dead(cell)) {
              dead_ends.add(Position.xy(x, y));
            }
            return false;
          },
        );
  ...
  int? relative_direction(Position? from, Position? to) {
    ...
    if (wrap_x &&
      ...
      if (from.x! < to.x!) {
        ...
        from!.x == to!.x &&
        (from.y == 0 || to.y == 0) &&
        (from.y == _height! - 1 || to.y == _height! - 1)) {
      if (from.y! < to.y!) {
        return N;
      } else {
        return S;
  ...
  void apply_move_at(int? x, int? y, int? direction) {
    ...
    switch (_symmetry) {
      case SymmetryType.x:
        _move_symmetrically_in_x(x, y, direction);
        break;
      case SymmetryType.y:
        _move_symmetrically_in_y(x, y, direction);
        break;
      case SymmetryType.xy:
        _move_symmetrically_in_xy(x, y, direction);
        break;
      case SymmetryType.radial:
        _move_symmetrically_radially(x, y, direction);
        break;
      default:
        break;
    }
...
//    }
  ...
  String to_s(dynamic options) {
    return to<formatters.ASCIIDelta, void>(FormatType.ascii, options)
        .toString();
  }
...
//        generated? ? "generated" : "not generated"]
    throw UnimplementedError('inspect');
  }
  ...
  bool weave_allowed /*?*/ (
      int? from_x, int? from_y, int thru_x, int thru_y, int? direction) {
    //#:nodoc:
    var movePos = move(thru_x, thru_y, direction);
    var nx2 = movePos.x!.toInt();
    var ny2 = movePos.y!.toInt();
    return (_cells![thru_y][thru_x] & UNDER == 0) &&
        valid(nx2.toInt(), ny2.toInt()) &&
        _cells![ny2][nx2] == 0;
  }

  List<int> perform_weave(
      int? from_x, int? from_y, int to_x, int to_y, int direction) {
    //#:nodoc:
    if (ruby.rand(2) == 0) {
      //# move under existing passage
      apply_move_at(to_x, to_y, direction << UNDER_SHIFT);
      apply_move_at(to_x, to_y, opposite(direction) << UNDER_SHIFT);
    } else {
      //# move over existing passage
  ...
  //# method.
  void _configure_symmetry() {
    //#:nodoc:
    if (_symmetry != /*none*/ null) {
      throw UnimplementedError("only :none symmetry is implemented by default");
    }
  }
  ...
  List<Position> _deadends_to_braid() {
    //#:nodoc:
    if (_braid == 0 /*.zero?*/) {
      return [];
    }
    ...
    return ends.take(count).toList()
      ..shuffle(ruby.getRandom()); // ends.shuffle[0,count];
  }

  //# Calculate the default entrance, by looking for the upper-leftmost point.
  Position _default_entrance() {
    //#:nodoc:
    Position result = Position.xy(
        0, 0); //# if every cell is masked, then 0,0 is as good as any!
    ruby.each_with_index<List<int>>(
      _cells!,
      (row, y) {
        //_cells.each_with_index do |row, y|
        return ruby.each_with_index<int>(
          row,
          (cell, x) {
            //row.each_with_index do |cell, x|
            if (_mask!.getCell(x, y)) {
              result = Position.xy(x - 1, y);
              return true;
            }
            return false;
          },
  ...
  Position _default_exit() {
    //#:nodoc:
    Position result = Position.xy(
        0, 0); //# if every cell is masked, then 0,0 is as good as any!
    ruby.each_with_index<List<int>>(_cells!.reversed, (List<int> row, y) {
      //_cells.reverse.each_with_index do |row, y|
  ...
  void _move_symmetrically_in_x(int? x, int y, int? direction) {
    //#:nodoc:
  ...
  void _move_symmetrically_in_y(int? x, int? y, int? direction) {
    //#:nodoc:
    if (direction == direction_under) {
  ...
  void _move_symmetrically_in_xy(int? x, int y, int? direction) {
    //#:nodoc:
    var row_width = _cells![y].length;
    if (direction == direction_under) {
  ...
  void _move_symmetrically_radially(int? x, int y, int? direction) {
    //#:nodoc:
    var row_width = _cells![y].length;
    if (direction == direction_under) {
