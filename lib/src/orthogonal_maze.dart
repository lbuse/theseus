...
class OrthogonalMaze extends Maze {
  ...
  Position finish /*!*/ () {
    ...
    switch (_symmetry) {
      case SymmetryType.x:
        if (even(_width!)) {
          connector(
              available_width! - 1, ruby.rand(available_height!), 0, 1, Maze.E);
        }
        break;
      case SymmetryType.y:
        if (even(_height!)) {
          connector(
              ruby.rand(available_width!), available_height! - 1, 1, 0, Maze.S);
        }
        break;
      case SymmetryType.xy:
        if (even(_width!)) {
          var xy = connector(available_width! - 1, ruby.rand(available_height!),
              0, 1, Maze.E)!;
        ...
        if (even(_height!)) {
          var xy = connector(ruby.rand(available_width!), available_height! - 1,
              1, 0, Maze.S)!;
      ...
      case SymmetryType.radial:
        if (even(_width!)) {
          _cells![available_height! - 1][available_width! - 1] |=
              Maze.E | Maze.S;
  ...
  Maze to_unicursal(MazeOptions options/*={}*/) {
    ...
    Maze unicursal = OrthogonalMaze(options);
    ...
    set(int x, int y, int direction, [bool recip = false]) {
      ...
      var nx = movePos.x;
      var ny = movePos.y;
      unicursal[Position.xy(x, y)] |= direction;
      if (recip) {
        unicursal[Position.xy(nx, ny)] |= opposite(direction);
      }
    }
    ...
    ruby.each_with_index<List<int>>(_cells!, (row, y) {
      ruby.each_with_index<int>(row, (cell, x) {
        ...
        if (cell & (Maze.N << Maze.UNDER_SHIFT) != 0) {
          unicursal[Position.xy(x2, y2)] |=
              (Maze.N | Maze.S) << Maze.UNDER_SHIFT;
          unicursal[Position.xy(x2 + 1, y2)] |=
              (Maze.N | Maze.S) << Maze.UNDER_SHIFT;
          unicursal[Position.xy(x2, y2 + 1)] |=
              (Maze.N | Maze.S) << Maze.UNDER_SHIFT;
          unicursal[Position.xy(x2 + 1, y2 + 1)] |=
              (Maze.N | Maze.S) << Maze.UNDER_SHIFT;
        } else if (cell & (Maze.W << Maze.UNDER_SHIFT) != 0) {
          unicursal[Position.xy(x2, y2)] |=
              (Maze.E | Maze.W) << Maze.UNDER_SHIFT;
          unicursal[Position.xy(x2 + 1, y2)] |=
              (Maze.E | Maze.W) << Maze.UNDER_SHIFT;
          unicursal[Position.xy(x2, y2 + 1)] |=
              (Maze.E | Maze.W) << Maze.UNDER_SHIFT;
    ...
    if (enter_at != null && exit_at != null) {
      ...
      if (enter_at.x! < exit_at.x!) {
        ...
        unicursal[Position.xy(enter_at.x, enter_at.y)] &= ~Maze.E;
        unicursal[Position.xy(enter_at.x! + 1, enter_at.y)] &= ~Maze.W;
      } else if (enter_at.y! < exit_at.y!) {
        unicursal[Position.xy(enter_at.x, enter_at.y)] &= ~Maze.S;
        unicursal[Position.xy(enter_at.x, enter_at.y! + 1)] &= ~Maze.N;
      }
    }
  ...
  _configure_symmetry() {
    ...
    if (_symmetry == SymmetryType.radial && _width != _height) {
      throw ArgumentError(
          "radial symmetrial is only possible for mazes where width == height");
    }
  }
...
//# Returns the maze rendered to a particular format. Supported
  ...
  V to<V, P>(FormatType format, [P? options] /*={}*/) {
    if (format == FormatType.ascii) {
      return formatters.ASCIIOrthogonal(this) as V;
    } else if (format == FormatType.png) {
      return formatters.PNGOrthogonal(
          this, options as formatters.PNGFormatterOptions) as V;
  ...
  static Maze generateStatic(MazeOptions options) {
    return OrthogonalMaze(options).generate();
  }
}
