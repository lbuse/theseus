...
class SigmaMaze extends Maze {
  ...
  int? _exit_wound(int entrance_direction, bool shifted) {
    //#:nodoc:
  ...
  @override
  bool weave_allowed(from_x, from_y, thru_x, thru_y, direction) {
    //#:nodoc:
  ...
  @override
  perform_weave(int? from_x, int? from_y, int to_x, int to_y, int direction) {
    //#:nodoc:
    ...
    apply_move_at(
        to_x, to_y, AXIS_MAP[shifted]![pass_thru]! << Maze.UNDER_SHIFT);
  ...
  V to<V, P>(FormatType format, [P? options]) {
    if (format == FormatType.ascii) {
      return formatters.ASCIISigma(this) as V;
    } else if (format == FormatType.png) {
      return formatters.PNGSigma(
