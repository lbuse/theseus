...
class Node {
  ...
  Node(this.point, this.under, this.path_cost, this.estimate,
...
class Astar extends Base {
  ...
  Astar(Maze maze /*, a=maze.start, b=maze.finish*/)
      : super(maze, maze.start(), maze.finish()) {
    //#:nodoc:
    _open = Node(_a, false, 0, _estimate(_a!), []);
    _visits = List.generate(
        _maze!.height!, (_) => List.generate(_maze!.width!, (_) => 0));
  }

  List<Position?> current_solution() {
    //#:nodoc:
    return List.from(_open!.history!)..add(_open!.point);
  }
  ...
  Position? step() {
    ...
    if (current.point == _b) {
      _open = null;
      _solution = List.from(current.history!)..add(_b);
    } else {
      _open = _open!.next;

      _visits[current.point!.y!.toInt()][current.point!.x!.toInt()] |=
          current.under ? 2 : 1;

      var cell = _maze![current.point!];

      List<int> directions = _maze!.potential_exits_at(
          current.point!.x!.toInt(), current.point!.y!.toInt());
      directions.forEach((dir) {
        ...
        if (cell & _try != 0) {
          ...
          var under =
              ((_maze![point] >> Maze.UNDER_SHIFT) & _maze!.opposite(dir) != 0);
          _add_node(point, under, current.path_cost + 1,
              List.from(current.history!)..add(current.point));
        }
      });
    }
  ...
  int _estimate(Position pt) {
    //#:nodoc:
    return Math.sqrt(Math.pow((_b!.x! - pt.x!), 2) /* **2 */ +
            Math.pow((_b!.y! - pt.y!), 2) /* **2 */)
        .toInt();
  }

  void _add_node(
      Position pt, bool under, int path_cost, List<Position?> history) {
    //#:nodoc:
    if (_visits[pt.y!.toInt()][pt.x!.toInt()] & (under ? 2 : 1) != 0) {
      return;
    }

    var node = Node(pt, under, path_cost, _estimate(pt), history);
  ...
  Position _move(Position pt, int direction) {
    //#:nodoc:
    return Position.xy(
        pt.x! + _maze!.dx(direction), pt.y! + _maze!.dy(direction));
  }
}
