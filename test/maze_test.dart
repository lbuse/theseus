...
void main() {
  ...
  test("maze_created_with_generate_is_identical_to_maze_created_with_step", () {
    srand(14);
    Maze maze1 = OrthogonalMaze.generateStatic(MazeOptions(width: 10));
    expect(true, equals(maze1.generated()));

    srand(14);
    Maze maze2 = OrthogonalMaze(MazeOptions(width: 10));
  ...
  test("apply_move_at_should_combine_direction_with_existing_directions", () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(MazeOptions(width: 10));

    maze.setCell(5, 5, /*Theseus::*/ Maze.E);
    maze.apply_move_at(5, 5, /*Theseus::*/ Maze.N);
    expect((/*Theseus::*/ Maze.N | /*Theseus::*/ Maze.E),
        equals(maze.getCell(5, 5)));
  });

  test(
      "apply_move_at_with_under_should_move_existing_directions_to_under_plane",
      () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(MazeOptions(width: 10));

    maze.setCell(5, 5, /*Theseus::*/ Maze.E);
    maze.apply_move_at(5, 5, Maze.direction_under);
    expect((/*Theseus::*/ Maze.E << /*Theseus::*/ Maze.UNDER_SHIFT),
        equals(maze.getCell(5, 5)));
  });

  test("apply_move_at_with_x_symmetry_should_populate_x_mirror", () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(
        MazeOptions(width: 10, symmetry: SymmetryType.x));
  ...
  test("apply_move_at_with_y_symmetry_should_populate_y_mirror", () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(
        MazeOptions(width: 10, symmetry: SymmetryType.y));
  ...
  test("apply_move_at_with_xy_symmetry_should_populate_xy_mirror", () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(
        MazeOptions(width: 10, symmetry: SymmetryType.xy));
  ...
  test("apply_move_at_with_radial_symmetry_should_populate_radial_mirror", () {
    Maze maze = /*Theseus::*/ OrthogonalMaze(
        MazeOptions(width: 10, symmetry: SymmetryType.radial));
  ...
  test("step_should_populate_current_cell_and_next_cell", () {
    OrthogonalMaze maze = /*Theseus::*/ OrthogonalMaze(MazeOptions(width: 10));
    ...
    expect(
        [nx, ny],
        equals([
          (maze.algorithm as RecursiveBacktracker).x,
          (maze.algorithm as RecursiveBacktracker).y
        ]));

    expect(maze.opposite(direction),
        equals(maze.getCell(nx!.toInt(), ny!.toInt())));
  });
}
